...
class VehicleControllerPointAtCarrot(Node):
    ...
    def main_timer_callback(self):
        ...
        if (self.pause == True):
            ...
            kp1 = 0.2
            kp2 = 0.2
            e = sqrt(((self.vehicle_point[0] - self.closest_point[0])^2) + ((self.vehicle_point[1] - self.closest_point[1])^2))
            if self.vehicle_heading_rad > self.closest_heading_rad:
                e = e * -1
            
            denominator = sqrt((self.vehicle_point[1])^2 + (self.vehicle_point[0])^2) * sqrt((self.closest_point[1])^2 + (self.closest_point[0])^2)

            cross_product = (self.closest_point[1]*self.vehicle_point[1])- (self.closest_point[1]*self.vehicle_point[0])
            if crossangle < 0: 
                e= e *-1
            ...
            new_steering_angle = new_steering_angle + kp1(self.closest_heading_rad - self.vehicle_heading_rad) + kp2*e
